...
use country\models\query\RegionQuery;
use Yii;
use country\models\Region;
use country\models\search\RegionSearch;
...
class RegionController extends Controller
{
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function($rule, $action){
                            return Yii::$app->user->can('createRegion');
                        }
                    ],
                    [
                        'actions' => ['update'],
                        ...
                        'matchCallback' => function($rule, $action){
                            ...
                            return Yii::$app->user->can('updateRegion', ['model' => $this->findModel($_GET['id'])]);
                        }
                    ],
                    [
                        'actions' => ['delete'],
                        ...
                        'matchCallback' => function($rule, $action){
                            if(!isset($_GET['id']))
                                throw new Exception("id parameter is missing");
                            return Yii::$app->user->can('deleteRegion', ['model' => $this->findModel($_GET['id'])]);
                        }
                    ],
                    [
                        'actions' => ['view'],
                        ...
                        'matchCallback' => function($rule, $action){
                            if(!isset($_GET['id']))
                                throw new Exception("id parameter is missing");
                            return Yii::$app->user->can('viewRegion', ['model' => $this->findModel($_GET['id'])]);
                        }
                    ],
                    [
                        'actions' => ['index'],
                        'allow' => true,
                        'matchCallback' => function($rule, $action){
                            return Yii::$app->user->can('indexRegion');
                        }
                    ],
                    [
    ...
    public function actionSelectRegion($country_id)
    {
        $models = Region::find()->countryQuery($country_id)
            ->orderByName()
            ->all();
