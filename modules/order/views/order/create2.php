...
if(isset($this->assetManager->bundles['all']))
    $this->clearAssetBundle(OrderAsset::class);
$orderAsset = OrderAsset::register($this);
?>
...
<div class="row orderCreate2" >
    <div class="col-lg-6 col-xs-6" >
        ...
        <?=$form->field($model, 'city_id',['parts'=>['{input}'=>
            (new City)->getWidgetSelectPicker($model, 'city_id',
                $model->region_id ?
                    City::find()->regionQuery($model->region_id):null
    ...
    <div class="col-lg-6 col-xs-6" >
        ...
        <div class="well" >
            ...
            <ul style="list-style-type: decimal;">
                ...
                foreach ($model->basketProducts as $orderProduct){
                    ?>
                    <li>
                        <a href="<?=Url::to(['/product/product/view', 'id'=>$orderProduct->product_id, 'title_url'=>$orderProduct->product->title_url])?>" style="color: #333;">
        ...
        <?= $form->field($model, 'online_payment_type',[
            'options' => ['class' => 'cardButtons', 'style'=>$model->payment_type==$model::PAYMENT_TYPE_ONLINE ? '':'display:none'],
            'template' => "{input}\n{error}",
            ...
            'item'=>function($index, $label, $name, $checked, $value) use($orderAsset)  {
                ...
                //$return .= ' '.$label;
                $return .= ' '.Html::img($orderAsset->baseUrl.'/images/'.$value.'.png', ['style'=>'height:30px;']);
                if($value==Order::ONLINE_PAYMENT_TYPE_PAYPAL)
                    $return.='<div class="hint-block" style="font-size: 12px; text-align: left">Fake Paypal account:<br>Login:buyer@sakuracommerce.com<br>Password:3@Rz%V3Gy"t^ctuS</div>';
                if($value==Order::ONLINE_PAYMENT_TYPE_CARD)
                    $return.='<div class="hint-block" style="font-size: 12px; text-align: left">Fake Visa account:<br>Card number:4032 0300 3777 5674<br>Expiration date:12/2023 &nbsp;&nbsp;&nbsp; CVV:123</div>';
                $return .= '</label>';
                return $return;
            },
        ]) ?>
        ...
        <div class="card" style="<?=$model->online_payment_type==$model::ONLINE_PAYMENT_TYPE_CARD ? '':'display:none'?>">
            <div class="cardWhite">
                <?php
                ...
                ?>
                <?=Html::img($orderAsset->baseUrl.'/images/'.$model::ONLINE_PAYMENT_TYPE_CARD.'.png', ['style'=>'height:40px;'])?>
