      ...
      const Plugin = function() {
        ...
        const createNavDom = (reinit) => {
          ...
          function createDom(menu, $container, level, title, backIndex) {
            ...
            // titles
            if (title) {
              $content.prepend(`<h2 data-title="${title}">All in ${title}</h2>`);
            }
                  ...
                  $a.on('click', (e) => {
                    e.stopPropagation();
                      ...
                      if (typeof window.router !=="undefined"){
                          window.router.push($item_link.attr('href'));
                          return false;
                      }
            ...
            if (level && typeof backIndex !== 'undefined') {
              if (Settings.insertBack !== false && Settings.levelOpen === 'overlap') {
                ...
                var backTitle = $content.parents('.nav-content').eq(0).find('h2').data('title');
                  if(backTitle=='All Categories')
                      backTitle = Back_to_all_categories;
                  else
                  if(backTitle=='Home')
                      backTitle = Back_to_home;
                  else
                    backTitle = $content.parents('.nav-content').eq(0).find('.all_link').eq(0).data('back-label');

                  //let $back = $(`<li class="nav-back"><a href="#">${Settings.labelBack || ''} to ${backTitle}<span></span></a></li>`);
                let $back = $(`<li class="nav-back"><a href="#">${backTitle}<span></span></a></li>`);

                $back.children('a').on('click', preventClick(() => closeLevel(level, backIndex)));
                $back.children('a').on('click', function(){

                    sessionStorage.setItem('level', level>1 ? level-1:0);
                    sessionStorage.removeItem('index-level-'+level);
        ...
        function checkboxChange() {
          ...
          if ($checkbox.prop('checked')) {
              ...
              sessionStorage.setItem('index-level-'+l, i);
              //console.log(sessionStorage);
          }
          else {
            closeLevel(l, i);
